...
class Scenario(basic_setup.BaseScenario):
    ...
    def create_energysystem(self, **parameters):
        ...
        # Create oil bus
        self.b_gas = Bus(label=AdvancedLabel("b_gas", type="Bus"), balanced=False)
    ...
    def add_technology(self, demand, timeseries, parameters):
        ...
        t_bhkw_net = Transformer(
            label=AdvancedLabel(
                f"transformer_from_{demand.name}_el", type="Transformer",
            ),
            inputs={
                b_bhkw_el: Flow(
        ...
        bhkw = Transformer(
            label=AdvancedLabel("bhkw", type="Transformer", tags=("bhkw",)),
            inputs={
                self.b_gas: Flow(
            ...
            outputs={
                b_bhkw_el: Flow(pf=parameters["General"]["pf_bhkw_el"]),
                self.sub_b_th: Flow(pf=pf_gas),
            },
            conversion_factors={
                b_bhkw_el: (parameters[self.name]["conversion_factor_el"] / 100),
                self.sub_b_th: (parameters[self.name]["conversion_factor_th"] / 100),
            },
        )
        ...
        gas_heating = Transformer(
            ...
            outputs={self.sub_b_th: Flow(variable_costs=parameters["Gas"]["opex"])},
            conversion_factors={self.sub_b_th: parameters["Gas"]["efficiency"] / 100},
        )
    ...
    def add_dynamic_parameters(cls, scenario, parameters):
        ...
        parameters[cls.name]["capex"] = parameters[cls.name]["capex"].new_child(
            {"value": str(int(capex))}
        )
        parameters[cls.name]["conversion_factor_el"] = parameters[cls.name][
            "conversion_factor_el"
        ].new_child({"value": str(int(eff))})
        return parameters
    ...
    def get_data_label(cls, nodes):
        if nodes[0].name == "bhkw":
            ...
            nodes[0].name.startswith("b_bhkw_el")
            and nodes[1] is not None
            and nodes[1].name.startswith("transformer_from")
        ):
            return "Stromgutschrift"
        elif nodes[1] is not None and (
            nodes[1].name == "bhkw" or nodes[1].name.endswith("gas_heating")
        ):
    ...
    def calculate_primary_factor_and_energy(cls, param_results, node_results):
        ...
        bhkw, b_bhkw_el = next(
            filter(
                lambda x: (
                    x[0].name == "bhkw"
                    and x[1] is not None
                    and x[1].name == "b_bhkw_el"
                ),
                param_results.keys(),
            )
        )
        b_demand_th, demand_node = next(
            filter(
                lambda x: (
                    x[0].name == "b_demand_th"
                    and x[1] is not None
        ...
        b_gas, _ = next(filter(lambda x: x[0].name == "b_gas", param_results.keys()))
        _, excess_node = next(
            filter(
