    ...
    def post(self, request, session):
        ...
        if form == "house":
            if "hh_instance" in request.POST:
                hh_id = get_object_or_404(Household, id=request.POST["hh_instance"])
...
class DemandDistrictView(TemplateView):
    ...
    def get_context_data(self, session):
        ...
        context["district_form"] = forms.DistrictListForm(session.current_district)
        context["wizard"] = Wizard([None] * 4, current=0)
        ...
        if any(key in request.POST for key in ("add_efh", "add_mfh", "trash")):
            ...
            if request.POST["district_status"] in ("new", "changed"):
                ...
                if request.POST["district_name"] == "":
                    ...
                    district = models.District.objects.get(pk=session.demand_id)
...
class TechnologyView(TemplateView):
    ...
    def get_context_data(self, session, **kwargs):
        ...
        if "bhkw" in app_settings.ACTIVATED_SCENARIOS:
            scenario = app_settings.SCENARIO_MODULES["bhkw"].Scenario
            if not scenario.is_available(demand):
                name = app_settings.SCENARIO_PARAMETERS["bhkw"]["LABELS"]["name"]
                technology_information["bhkw"][
                    "warning"
        ...
        if "bio_bhkw" in app_settings.ACTIVATED_SCENARIOS:
            scenario = app_settings.SCENARIO_MODULES["bio_bhkw"].Scenario
            if not scenario.is_available(demand):
                name = app_settings.SCENARIO_PARAMETERS["bio_bhkw"]["LABELS"]["name"]
                technology_information["bio_bhkw"][
                    "warning"
                ] = f"{name} ist nicht in nötiger Größe verfügbar"
                ...
                technology_information["bio_bhkw"][
                    "warning"
                ] = app_settings.SCENARIO_PARAMETERS["bio_bhkw"]["LABELS"]["warning"]
        ...
        if "pv_heatpump" in app_settings.ACTIVATED_SCENARIOS:
            if demand.contains_radiator():
                technology_information["pv_heatpump"][
                    "warning"
                ] = app_settings.SCENARIO_PARAMETERS["pv_heatpump"]["LABELS"]["warning"]
        ...
        context["wizard"] = Wizard(
            ...
            current=1,
            screen_reader_for_current=("Sie sind auf der Seite Technologie-Auswahl"),
        )
        return context
...
class ParameterView(TemplateView):
    ...
    def get_scenario_parameters(session):
        ...
        for scenario in scenarios:
            ...
            scenario_parameters = scenario.module.Scenario.add_dynamic_parameters(
                scenario, scenario_parameters
            )
            parameters.append((scenario.name, scenario_parameters))
        return parameters
    ...
    def post(self, request, session):
        ...
        for scenario in session.scenarios:
            scenario.parameter.update(parameter_form.prepared_data(scenario.name))

        session.changed_parameters = forms.ParameterForm.get_changed_parameters(
...
class ResultView(TemplateView):
    ...
    def get_context_data(self, result_ids, **kwargs):
        ...
        aggregated_results = results.ResultAggregations(result_ids, aggregations)
    ...
    def get(self, request, *args, **kwargs):
        ...
        if result_ids is not None:
            ...
            if len(results_not_found) > 0:
                return self.render_to_response({"results_not_found": results_not_found})
        ...
        pending = any([scenario.is_pending() for scenario in session.scenarios])
        if pending:
            # Render pending simulation:
            return redirect("stemp:pending")

        result_ids = [
            sc.result_id for sc in session.scenarios if sc.result_id is not None
        ]
        if len(result_ids) == 0:
            return self.render_to_response({})
...
class PendingView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        filename = os.path.join(BASE_DIR, "stemp", "texts", "energiespartipps.csv")
        data = pandas.read_csv(filename, index_col=0, sep=";")
        return {
            "tipps": OrbitWidget(
                "Energiespartipps",
                [
