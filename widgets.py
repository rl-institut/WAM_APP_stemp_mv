...
class DynamicWidgetMixin(object):
    ...
    def __init__(self, url, attrs, widget, dynamic_function_name, dynamic_id, initial):
        self.url = url
        self.widget = widget
        ...
        if "onChange" in attrs:
            warnings.warn(
                '"OnChange" event will be overwritten by ' "SelectGraphWidget"
            )
...
class DynamicSelectWidget(DynamicWidgetMixin, Select):
    def __init__(
        ...
        DynamicWidgetMixin.__init__(
            self, dynamic_url, attrs, Select, dynamic_function_name, dynamic_id, initial
        )
